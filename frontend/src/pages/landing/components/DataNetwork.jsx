...
function createNodes(w, h) {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const layer = Math.floor(s * 3) + 1; // 1, 2, 3 layers

    if (i === 0) {
      nodes.push({ x: centerX, y: centerY, vx: 0, vy: 0, r: 3.5, layer: 0, phase: 0, speed: 0, isHub: true, connections: [] });
      continue;
    }

    // Concentric orbits distribution using absolute CSS pixels (preserving aspect ratio 1:1)
    const orbitRadius = (layer / 3) * maxRadius + (s * 25); // Add a small random offset to break perfect rings
    const angle = s2 * Math.PI * 2 + (i * 0.05);
  ...
  // Symmetrical uniform distance connections in CSS pixels
  const maxDist = maxRadius * 0.35;
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      const dx = nodes[i].x - nodes[j].x;
      const dy = nodes[i].y - nodes[j].y;
      const d = Math.hypot(dx, dy);
      if (d < maxDist && (nodes[i].layer === nodes[j].layer || Math.abs(nodes[i].layer - nodes[j].layer) === 1)) {
...
export default function DataNetwork() {
  ...
  useEffect(() => {
    ...
    function draw(t) {
      ...
      // ─── Draw Symmetrical Radar Sweep ───
      const radarAngle = (t * 0.00025) % (Math.PI * 2);
      ...
      // 2. Fading radar sweep tail (wedges)
      const sweepSteps = 35;
      const sweepAngleSpan = 0.45;
      for (let i = 0; i < sweepSteps; i++) {
        const stepAngle = radarAngle - (i / sweepSteps) * sweepAngleSpan;
        const opacity = (1 - i / sweepSteps) * 0.065;
        ctx.beginPath();
        ctx.moveTo(centerX, centerY);
        ctx.arc(centerX, centerY, maxRadius, stepAngle - 0.015, stepAngle + 0.015, false);
        ctx.lineTo(centerX, centerY);
        ctx.fillStyle = `rgba(34, 211, 238, ${opacity})`;
        ctx.fill();
      }

      // 3. Leading radar sweep line
      const lx = centerX + Math.cos(radarAngle) * maxRadius;
      const ly = centerY + Math.sin(radarAngle) * maxRadius;
      ctx.beginPath();
      ctx.moveTo(centerX, centerY);
      ctx.lineTo(lx, ly);
      ...
      for (const node of nodes) {
        ...
        if (node.isHub) {
          ...
          grd.addColorStop(0.3, 'rgba(34,211,238,0.07)');
